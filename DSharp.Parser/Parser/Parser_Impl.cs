...
namespace DSharp.Parser
{
	...
	public partial class DParser
	{
		...
		DModule Root()
		{
			...
			// Only one namespace declaration possible!
			if (laKind == Namespace)
			{
				module.Description = GetComments();
				module.NamespaceOverride= NamespaceOverride();
				module.Description += CheckForPostSemicolonComment();

				if (module.NamespaceOverride.Namespace!=null)
					module.ModuleName = module.NamespaceOverride.Namespace.ToString();
				module.NamespaceOverride.ParentNode = doc;
			}
		...
		NamespaceStatement NamespaceOverride()
		{
			Expect(Namespace);
			var ret = new NamespaceStatement { StartLocation=t.Location };
			LastParsedObject = ret;
			ret.Namespace = ModuleFullyQualifiedName();
			if (Expect(Semicolon))
		...
		IStatement Statement(bool BlocksAllowed = true, bool EmptyAllowed = true, IBlockNode Scope = null, IStatement Parent=null)
		{
			...
			else if (laKind == (Do))
			{
				...
				var dbs = new WhileStatement() { IsDoWhile=true, StartLocation = t.Location, Parent = Parent };
				LastParsedObject = dbs;
				dbs.ScopedStatement = Statement(Scope: Scope, Parent: dbs);
